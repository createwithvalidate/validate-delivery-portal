...
const maxDownloadBytes = Number(process.env.DOWNLOAD_FILE_MAX_BYTES || 250 * 1024 * 1024);
...
function safeDownloadUrl(rawUrl) {
  ...
  if (isPrivateDownloadHost(url.hostname)) throw new Error("Download URL is not allowed.");
  return url;
}

async function fetchDownloadFile(urlValue, fileName = "download.mp4") {
  const url = safeDownloadUrl(urlValue);
  ...
  if (data.length > maxDownloadBytes) {
    throw new Error("This file is too large to download automatically.");
  }
  return {
    name: sanitizeDownloadName(fileName, "download.mp4"),
    data,
    contentType: downloadResponse.headers.get("content-type") || "application/octet-stream",
  };
}

async function sendDownloadFile(request, response, token) {
  if (!token) {
    sendJson(response, 401, { error: "Sign in again before downloading files." });
    return;
  }

  await getUser(token);
  const body = await parseRequestBody(request);
  if (!body.url) throw new Error("No download file was selected.");
  const file = await fetchDownloadFile(body.url, body.fileName);
...
module.exports = async function handler(request, response) {
  ...
  if (request.method === "POST" && url.searchParams.get("downloadFile") === "1") {
    try {
      await sendDownloadFile(request, response, authToken(request));
    } catch (error) {
      sendJson(response, error.statusCode || 400, { error: error.message || "Download could not be created." });
    }
    return;
  }
