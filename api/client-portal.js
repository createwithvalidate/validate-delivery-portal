...
async function fetchDownloadFile(file, index) {
  ...
  if (!downloadResponse.ok) {
    throw new Error(`Could not fetch ${file.fileName || `file ${index + 1}`}.`);
  }

  const data = Buffer.from(await downloadResponse.arrayBuffer());
  return {
    name: sanitizeDownloadName(file.fileName, `download-${index + 1}.mp4`),
    data,
  };
}

async function sendDownloadZip(request, response, token) {
  if (!token) {
  ...
  const projectName = sanitizeDownloadName(body.projectName, "validate-downloads")
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
module.exports = async function handler(request, response) {
  ...
  if (request.method === "POST" && url.searchParams.get("downloadZip") === "1") {
    try {
      await sendDownloadZip(request, response, authToken(request));
    } catch (error) {
      sendJson(response, error.statusCode || 400, { error: error.message || "Download zip could not be created." });
    }
    return;
  }
